import redis
from scrapy_plus.conf.settings import REDIS_HOST, REDIS_PORT, REDIS_DB, FP_PERSIST, REDIS_FP_KEY
...
class RedisStatsCollector(object):
    ...
    def clear(self):
        ...
        self.redis.delete(self.request_nums_key, self.response_nums_key,
                          self.repeat_request_nums_key,)
        # 判断是否清空指纹集合
        if not FP_PERSIST:  # not True 不持久化，就清空指纹集合
            self.redis.delete(REDIS_FP_KEY)
